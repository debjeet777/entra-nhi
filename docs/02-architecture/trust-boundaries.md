...
| **Untrusted or less-trusted side** | Microsoft Entra identity provider and Microsoft Graph for V1; future explicitly enabled optional sources such as Azure Resource Manager or licensed telemetry |
...
| **Security assumptions** | Microsoft services operate correctly and return documented response schemas; responses reflect the actual tenant state at observation time; TLS protects authenticated transport/channel integrity according to the selected provider/client implementation, but transport protection alone does not prove semantic correctness or cryptographic provenance of individual provider objects |
| **Validation requirements** | EntraNHI MUST NOT assume all tenants expose identical APIs, licensing tiers, or endpoint behavior [CAP-003, INV-07]; response schemas MUST be validated against documented expectations; unexpected or malformed responses MUST produce structured failure diagnostics, not silent inference; capability detection MUST distinguish unavailable data from absent data [INV-07] |
| **Failure behavior** | Malformed or unexpected provider responses produce structured collection errors and preserve applicable capability/completeness/failure context; authentication failures remain explicit and preserve their failure context; transient failures use bounded retry with backoff. Operational failures do not themselves select an assessment state and never silently become PASS or tenant-security FAIL. When a condition merely prevents assessment, applicable evaluation/rule semantics and later failure-mapping mechanics determine the legitimate handling. A separate rule may explicitly evaluate the condition as assessment data under a documented requirement, in which case normal deterministic rule semantics govern its state. |
...
| **Data crossing the boundary** | Runtime authentication material required by the selected validated mechanism, tenant context identifiers, authorization metadata; exact token caching/storage and in-memory protection remain deferred |
...
| **Prohibited behavior** | Authentication material MUST NOT be persisted by assessment-domain or artifact paths and MUST NOT appear in output artifacts or logs; any future authentication cache/storage requires the separate decision deferred by authentication-authorization.md §8.3; the authentication boundary MUST NOT grant itself additional tenant privileges; authentication success MUST NOT bypass per-capability authorization checks; AI/LLM MUST NOT influence authentication or authorization decisions [INV-13] |
...
| **Untrusted or less-trusted side** | Microsoft Graph API for V1; future explicitly enabled optional enrichment sources such as Azure Resource Manager or licensed telemetry endpoints |
...
| **Validation requirements** | All V1 tenant-data collection currently in scope MUST occur through documented, approved Microsoft Graph endpoints [INV-15]; collection MUST NOT perform write, delete, or administrative operations [INV-01]; collected data MUST be validated against expected schema before normalization; optional enrichment APIs MUST be isolated behind their own collector/adaptor boundary, explicitly enabled, and MUST NOT become V1 or implicit dependencies [collection-architecture.md §1]; permission requirements MUST be documented per capability [SEC-008] |
| **Failure behavior** | API unavailability, throttling, transient errors, and authorization denial produce structured failure/capability context [collection-architecture.md §10]; they do not directly assign a rule state and never silently become PASS or tenant-security FAIL. Applicable evaluation/rule semantics and later failure-mapping mechanics determine any RuleEvaluation state; partial collection MUST remain visible downstream [INV-05, INV-07, INV-14]. |
...
| **Failure behavior** | Collection failures are recorded and propagated with applicable capability/completeness/failure context through SourceObservation envelopes; a failed collector MUST NOT silently return an empty successful collection [collection-architecture.md §10]. Collectors do not assign rule states. When a condition merely prevents assessment, applicable evaluation/rule semantics and later failure-mapping mechanics determine the legitimate handling, including whether a RuleEvaluation is produced and, if so, its state. A separate rule may explicitly evaluate the condition as assessment data under a documented requirement, in which case normal deterministic rule semantics govern its state [INV-14]. |
...
| **Failure behavior** | Normalization-input validation failures produce explicit structured failure diagnostics and preserve applicable context; conflicting observations MUST remain detectable and MUST NOT be silently resolved by heuristic inference [INV-02]. Normalization failures do not directly assign rule states and MUST NOT silently produce PASS or tenant-security FAIL. |
...
| **Failure behavior** | A rule execution exception after a valid evaluation context is established may produce ERROR according to rule semantics, and ERROR MUST NOT be converted to PASS or FAIL to simplify reporting. Invalid shared normalized state, inconsistent graph state, tenant mismatch, or invalid configuration that prevents trustworthy evaluation construction is an integrity/construction failure: it fails visibly, produces no fabricated RuleEvaluation, and is not automatically mapped to an assessment state. Exact fatal-versus-isolated handling remains deferred [rule-engine.md §4.3, §13]. |
...
| **Failure behavior** | Evidence fabrication attempts, impossible or broken evidence linkage, and tenant-context mismatch are integrity/construction failures that fail safely and visibly [INV-10]. They produce no fabricated or replacement RuleEvaluation, are not automatically mapped to any assessment state, and MUST NOT silently convert an assessment into a successful/secure result. Exact fatal-versus-isolated handling remains deferred. |
...
| **Failure behavior** | Output-generation failures MUST produce visible failure diagnostics; partial artifacts MUST be cleaned up or quarantined; failed temporary files MUST NOT persist as apparently-complete assessment artifacts; filesystem permission errors MUST produce explicit failure, not silent success |
...
| **Failure behavior** | Tenant-context mismatch and cross-tenant data mixing are integrity/construction failures that fail explicitly and visibly; they produce no fabricated RuleEvaluation and are not automatically mapped to any assessment state. Mismatched tenant context MUST NOT silently produce assessment output; exact fatal-versus-isolated handling remains deferred. |
...
Data originating from Microsoft Graph, or from a future explicitly enabled optional source such as Azure Resource Manager or licensed telemetry, is observational input to the assessment pipeline. Microsoft Graph is the V1 tenant-data source; optional sources are not V1 dependencies. Provider data is NOT automatically authoritative or complete. EntraNHI produces its own deterministic evaluation based on collected observations. Provider-originated and tenant-controlled strings remain untrusted data in all rendering contexts. [INV-07, INV-15]

### Authentication success does NOT imply authorization for every capability

Authentication establishes identity and access context. Authorization is evaluated per required collection capability. A successfully authenticated principal may lack authorization for specific capabilities. Authorization denial remains explicit and never silently becomes PASS or tenant-security FAIL; applicable evaluation/rule semantics and later failure-mapping mechanics determine any resulting evaluation state. [INV-08]
...
| 12 — Tenant isolation | INV-10, INV-G1, findings-evidence.md §12 |
